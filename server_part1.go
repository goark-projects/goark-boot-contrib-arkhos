...
func (s *EmbeddedServer) Start(ctx context.Context) error {
	...
	if result.serveStopped {
		...
		return arkerrors.Wrap(
			...
			result.err,
			"arkhos server stopped during startup",
		)
	}
...
func (s *EmbeddedServer) finishStart(
	server ManagedServer,
	listener net.Listener,
	errCh chan error,
) {
...
func (s *EmbeddedServer) beginStop() (ManagedServer, net.Listener, chan error, bool, error) {
	...
	default:
		return nil, nil, nil, false, arkerrors.New(
			arkerrors.CodeConflict,
			"embedded server is busy",
		)
	}
}
	...
	for _, deployment := range s.deployments {
		...
		if _, err := s.container.Deploy(ctx, deployment); err != nil {
			return arkerrors.Wrap(
				arkerrors.CodeLifecycle,
				err,
				"failed to deploy arkarta servlet application",
			)
		}
	}
	return nil
...
func waitServer(ctx context.Context, errCh <-chan error) error {
	...
	case <-ctx.Done():
		return arkerrors.Wrap(
			arkerrors.CodeLifecycle,
			ctx.Err(),
			"waiting arkhos server shutdown canceled",
		)
	}
}
